...
from tsrl_lite.networks import TorchDLinearActorCriticNetwork


if torch is not None and TorchDLinearActorCriticNetwork is not None:

    @register_agent("torch-dlinear-ppo")
    class TorchDLinearPPOAgent:
        ...
        def __init__(
            ...
            if len(encoder.observation_shape) != 2:
                raise ValueError("torch-dlinear-ppo requires a sequence-shaped encoder output")
            sequence_length, input_dim = encoder.observation_shape
            self.sequence_length = sequence_length
            self.input_dim = input_dim

            torch.manual_seed(seed)
            self.network = TorchDLinearActorCriticNetwork(
                input_dim=input_dim,
            ...
            backbone_parameters = []
            if individual:
                backbone_parameters.extend(self.network.seasonal_linears.parameters())
                backbone_parameters.extend(self.network.trend_linears.parameters())
            else:
                backbone_parameters.extend(self.network.seasonal_linear.parameters())
                backbone_parameters.extend(self.network.trend_linear.parameters())
            backbone_parameters.extend(self.network.feature_projection.parameters())
            backbone_parameters.extend(self.network.norm.parameters())
            self.optimizer = optim.Adam(
                [
                    {"params": backbone_parameters, "lr": policy_lr},
        ...
        def load(self, path: str) -> None:
            network, optimizer_state = TorchDLinearActorCriticNetwork.load(path, map_location=self.device)

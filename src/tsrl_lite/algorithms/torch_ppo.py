...
from tsrl_lite.networks import TorchGRUActorCriticNetwork
from tsrl_lite.algorithms.torch_ppo_common import prepare_torch_ppo_batch, run_torch_ppo_update


if torch is not None and TorchGRUActorCriticNetwork is not None:

    @register_agent("torch-gru-ppo")
    class TorchGRUPPOAgent:
        ...
        def __init__(
            ...
            if len(encoder.observation_shape) != 2:
                raise ValueError("torch-gru-ppo requires a sequence-shaped encoder output")
            sequence_length, input_dim = encoder.observation_shape
            self.sequence_length = sequence_length
            self.input_dim = input_dim

            torch.manual_seed(seed)
            self.network = TorchGRUActorCriticNetwork(
                input_dim=input_dim,
            ...
            self.optimizer = optim.Adam(
                [
                    {"params": self.network.gru.parameters(), "lr": policy_lr},
        ...
        def load(self, path: str) -> None:
            network, optimizer_state = TorchGRUActorCriticNetwork.load(path, map_location=self.device)

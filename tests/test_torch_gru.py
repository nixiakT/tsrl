...
class TorchGRUSmokeTest(unittest.TestCase):
    def test_torch_gru_update_reports_stability_metrics(self) -> None:
        encoder = SequenceWindowEncoder(window_size=8, agent_feature_dim=1)
        agent = TorchGRUPPOAgent(
            encoder=encoder,
            action_dim=3,
            gamma=0.99,
            gae_lambda=0.95,
        ...
        observations = np.random.default_rng(13).normal(
        ...
        self.assertIn("update_steps", metrics)
    ...
    def test_torch_gru_training_produces_pt_checkpoint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            config_path = tmp_path / "torch_gru_config.json"
            output_dir = tmp_path / "run"
            config = {
                "experiment_name": "torch_gru_smoke",
                "seed": 19,
                "data": {
                    "source": "synthetic",
                    "window_size": 20,
            ...
            self.assertEqual(summary["agent"], "torch-gru-ppo")
            self.assertIn("train_update_metrics", summary)
            self.assertIn("explained_variance", summary["train_update_metrics"]["overall"])
            self.assertIn("early_stop_triggered", summary["train_update_metrics"]["overall"])
